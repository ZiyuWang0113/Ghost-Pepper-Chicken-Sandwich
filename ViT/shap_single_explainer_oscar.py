...
def save_checkpoint(explainer, segments, shap_map, segment_values, pred_probs, class_idx, filename):
...
def log_memory_usage():
    ...
    print(f"Memory Usage - RSS: {memory_info.rss / 1e9:.2f} GB, VMS: {memory_info.vms / 1e9:.2f} GB")
...
if torch.cuda.is_available():
    ...
    # Set cudnn benchmark for faster training if input sizes don't change
...
# Improved SHAP class for deepfake detection with multi-GPU support
class ImprovedShapExplainer:
    def __init__(self, model, image_processor, device, 
        ...
        self.num_gpus = torch.cuda.device_count() if self.multi_gpu and torch.cuda.is_available() else 1
        if self.multi_gpu and torch.cuda.is_available():
            ...
            if self.num_gpus > 1:
                ...
                print("Multi-GPU processing requested but only 1 GPU available. Falling back to single GPU.")
        else:
            self.multi_gpu = False
            if not torch.cuda.is_available():
                print("CUDA not available. Multi-GPU processing disabled.")
    ...
    def _predict_batch(self, masked_images, gpu_id=0):
        """Get model predictions for a batch of masked images on a specific GPU"""
        results = []
        
        # Process in smaller batches to manage memory
        for i in range(0, len(masked_images), self.batch_size):
            batch = masked_images[i:i + self.batch_size]
            
            # Process images in batch
            batch_inputs = self.image_processor(images=batch, return_tensors="pt")
            
            # Move to appropriate device
            device = f'cuda:{gpu_id}' if self.multi_gpu else self.device
            batch_inputs = {k: v.to(device) for k, v in batch_inputs.items()}
            
            # Select the right model based on GPU
            model = self.models[gpu_id] if self.multi_gpu else self.model
            
            # Get predictions
            with torch.no_grad():
                batch_outputs = model(**batch_inputs)
                batch_probs = F.softmax(batch_outputs.logits, dim=1).cpu().numpy()
    ...
    def generate_shap_values(self, target_class):
        """Generate SHAP values for the image using a partition-based approach with multi-GPU support"""
        ...
        def f(masks):
            ...
            # Generate masked images for all samples
            masked_imgs = []
            for mask in masks:
                masked_imgs.append(self.apply_mask(mask.astype(bool)))
            
            if self.multi_gpu and self.num_gpus > 1:
                # Split the work across GPUs
                n_samples_per_gpu = len(masked_imgs) // self.num_gpus + 1
                
                # Create batches for each GPU
                gpu_batches = []
                for i in range(self.num_gpus):
                    start_idx = i * n_samples_per_gpu
                    end_idx = min(start_idx + n_samples_per_gpu, len(masked_imgs))
                    if start_idx < end_idx:  # Skip empty batches
                        gpu_batches.append((masked_imgs[start_idx:end_idx], i))
                
                # Process batches in parallel across GPUs
                results = Parallel(n_jobs=self.num_gpus)(
                    delayed(self._predict_batch)(batch, gpu_id) 
                    for batch, gpu_id in gpu_batches
                )
                
                # Flatten results and extract the target class probabilities
                all_preds = []
                for batch_result in results:
                    all_preds.extend([pred[target_class] for pred in batch_result])
                
                return np.array(all_preds)
            else:
                # Single GPU processing
                all_preds = self._predict_batch(masked_imgs)
                return np.array([pred[target_class] for pred in all_preds])
        ...
        try:
            ...
            # Log memory after SHAP calculation
            log_memory_usage()
    ...
    def visualize_explanation(self, segments, shap_map, segment_values, 
        ...
        ax3.set_title(f'SHAP Values Heatmap\nClass: {class_name}, Prob: {probability:.4f}', 
                      fontsize=14)
        ax3.axis('off')
    ...
    def create_overlay_visualization(self, shap_map, class_name, probability, 
        ...
        ax.imshow(overlay)
        ax.set_title(f'SHAP Explanation Overlay\nClass: {class_name}, Probability: {probability:.4f}', 
                     fontsize=14)
        ax.axis('off')
...
def main():
    ...
    try:
        ...
        # Initialize SHAP explainer with multi-GPU support
        ...
        for class_idx in classes_to_analyze:
            ...
            # Update current_results for checkpointing
